...
<div class="app-shell">
    <aside class="sidebar" id="sidebar">
        ...
        <nav aria-label="Dashboard navigation">
            ...
            <a class="nav-item" href="#firewall" data-section="firewall"><span class="nav-icon">⚿</span>Firewall</a>
    ...
    <div class="main-column">
        ...
        <main class="content" id="dashboard">
            ...
            <section class="dashboard-section is-visible" data-panel="dashboard">
                ...
                <div class="status-grid status-grid-four service-cards">
            ...
            <section class="dashboard-section" data-panel="logs"><div class="section-heading"><div><h2>Logs</h2><p class="muted">Output is escaped before rendering.</p></div></div><section class="panel"><div class="toolbar"><label>Source <select data-log-type><option value="nginx-error">Nginx error</option><option value="nginx-access">Nginx access</option><option value="audit">Audit</option><option value="login">Login</option><option value="system">System</option></select></label><button class="button button-secondary" type="button" data-load-logs>Load last 100</button></div><pre class="log-viewer" data-logs>Choose a log source to load it.</pre></section></section>
            <section class="dashboard-section" data-panel="backup">
                <div class="section-heading"><div><h2>Backup</h2><p class="muted">Create and manage server backup archives.</p></div><button class="button button-ghost" type="button" data-backup-refresh>Refresh</button></div>
                <section class="panel">
                    <p class="muted">The backup archive includes the registered server state and configuration. Restore remains available from the CLI after review.</p>
                    <button class="button button-secondary" type="button" data-action="backup-all" data-confirm="Create a full server backup now?">Create full backup</button>
                </section>
                <section class="panel backup-list-panel">
                    <div class="panel-heading"><div><h3>Backup list</h3><p class="muted">Deleting an archive permanently removes the backup file from this server.</p></div></div>
                    <div class="table-wrap"><table><thead><tr><th>Backup</th><th>Size</th><th>Created</th><th>Actions</th></tr></thead><tbody data-backups><tr><td colspan="4" class="empty-state">Loading…</td></tr></tbody></table></div>
                </section>
            </section>
